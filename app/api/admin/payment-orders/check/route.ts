import { randomUUID } from "crypto";
import { type NextRequest } from "next/server";
import { apiError, apiSuccess } from "@/lib/core/api-response";
import { handleRouteError } from "@/lib/api/route-helpers";
import { requireAdminApiAuth } from "@/lib/auth/admin-auth";
import { checkRateLimit } from "@/lib/core/rate-limit";
import { env } from "@/lib/core/env";
import { getClientIp } from "@/lib/core/utils";
import { paymentOrderService } from "@/lib/services/payment-order-service";

export const runtime = "nodejs";

export async function POST(request: NextRequest) {
  const requestId = request.headers.get("x-request-id") ?? randomUUID();
  try {
    const ip = getClientIp(request.headers);
    const rateLimit = checkRateLimit(`admin-payment-orders-check:${ip}`, env.ADMIN_RATE_LIMIT, 60_000);
    if (!rateLimit.allowed) {
      return apiError(requestId, "请求过于频繁，请稍后重试", "RATE_LIMITED", 429);
    }

    await requireAdminApiAuth(request);
    const body = (await request.json()) as { keyword?: string };
    const order = await paymentOrderService.findByKeyword(body.keyword ?? "");
    const data = order.status === "pending" ? await paymentOrderService.reconcilePendingOrder(order.orderId) : order;
    return apiSuccess(requestId, data);
  } catch (error) {
    return handleRouteError(error, requestId);
  }
}
